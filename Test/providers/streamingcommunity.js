  ...
  "src/formatter.js"(exports2, module2) {
    ...
    function formatStream2(stream, providerName) {
      ...
      if (!language) {
        ...
        else language = "\u{1F1EE}\u{1F1F9} \u{1F1EC}\u{1F1E7}";
      }
      let details = [];
      ...
      const is\u{1F4C1}Provider = String(providerName || "").toLowerCase() === "\u{1F4C1}" || String((stream == null ? void 0 : stream.name) || "").toLowerCase().includes("\u{1F4C1}");
      if (is\u{1F4C1}Provider && !finalHeaders) {
      ...
      if (!is\u{1F4C1}Provider && shouldForceNotWebReady) {
...
// src/\u{1F4C1}/index.js
function get\u{1F4C1}BaseUrl() {
...
function extractEmbedSrcFromApiPayload(payload) {
  ...
  try {
    return new URL(rawSrc, get\u{1F4C1}BaseUrl()).toString();
  } catch (e) {
    return null;
  }
}
...
function getTmdbId(imdbId, type) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      console.error("[\u{1F4C1}] Conversion error:", e);
      return null;
    }
  });
}
function getMetadata(id, type) {
  return __async(this, null, function* () {
    try {
      ...
      console.error("[\u{1F4C1}] Metadata error:", e);
      return null;
    }
  });
}
    ...
    if (normalizedType === "movie" && guardahd && typeof guardahd.getStreams === "function") {
      checks.push(
        guardahd.getStreams(id, normalizedType, season, episode).then((streams) => Array.isArray(streams) && streams.length > 0).catch((e) => {
          console.warn("[\u{1F4C1}] GuardaHD fallback check failed:", e);
          return false;
        })
      );
    }
    if (normalizedType === "tv" && guardaserie && typeof guardaserie.getStreams === "function") {
      checks.push(
        guardaserie.getStreams(id, normalizedType, season, episode, providerContext).then((streams) => Array.isArray(streams) && streams.length > 0).catch((e) => {
          console.warn("[\u{1F4C1}] Guardaserie fallback check failed:", e);
          return false;
        })
      );
    }
...
function getStreams(id, type, season, episode, providerContext = null) {
  return __async(this, null, function* () {
    ...
    const baseUrl = get\u{1F4C1}BaseUrl();
    ...
    if (contextTmdbId) {
      ...
      if (convertedId) {
        console.log(`[\u{1F4C1}] Converted ${id} to TMDB ID: ${convertedId}`);
        tmdbId = convertedId;
      } else {
        console.warn(`[\u{1F4C1}] Could not convert IMDb ID ${id} to TMDB ID.`);
      }
    }
    let metadata = null;
    try {
      ...
      console.error("[\u{1F4C1}] Error fetching metadata:", e);
    }
    ...
    try {
      console.log(`[\u{1F4C1}] Fetching API: ${apiUrl}`);
      const response = yield fetch(apiUrl, {
        headers: commonHeaders
      });
      if (!response.ok) {
        console.error(`[\u{1F4C1}] Failed to fetch page: ${response.status}`);
        return [];
      }
      const apiPayload = yield response.json().catch(() => null);
      const embedUrl = extractEmbedSrcFromApiPayload(apiPayload);
      if (!embedUrl) {
        console.log("[\u{1F4C1}] Could not find embed src in API payload");
        return [];
      }
      if (providerContext == null ? void 0 : providerContext.proxyUrl) {
        const rawPageUrl = url.endsWith("/") ? url : `${url}/`;
        console.log(`[\u{1F4C1}] Proxy enabled, returning raw page URL: ${rawPageUrl}`);
        const result = {
          name: `\u{1F4C1}`,
          title: finalDisplayName,
          url: rawPageUrl,
          easyProxySourceUrl: rawPageUrl,
          // Stremio addon uses EasyProxy path for \u{1F4C1}, so expose default quality here too.
          quality: "1080p",
          type: "direct",
          behaviorHints: {
            notWebReady: false
        ...
        return [formatStream(result, "\u{1F4C1}")].filter((s) => s !== null);
      }
      console.log(`[\u{1F4C1}] Fetching embed: ${embedUrl}`);
      const embedResponse = yield fetch(embedUrl, {
        headers: getEmbedHeaders(embedUrl)
      });
      if (!embedResponse.ok) {
        console.error(`[\u{1F4C1}] Failed to fetch embed: ${embedResponse.status}`);
        return [];
      }
      ...
      if (masterPlaylist) {
        ...
        console.log(`[\u{1F4C1}] Final stream URL: ${streamUrl}`);
        let quality = "1080p";
        try {
          ...
          if (playlistResponse.ok) {
            ...
            if (!hasItalian && !originalLanguageItalian) {
              console.log(`[\u{1F4C1}] No Italian audio found. Checking fallback.`);
              const fallbackOk = yield hasGuardaFallbackResults(id, normalizedType, resolvedSeason, episode, providerContext);
              if (!fallbackOk) return [];
            }
          }
        } catch (e) {
          console.warn(`[\u{1F4C1}] Playlist pre-check failed, continuing:`, e);
        }
        ...
        console.log("[\u{1F4C1}] Could not find playlist info in HTML");
        return [];
      }
    } catch (error) {
      console.error("[\u{1F4C1}] Error:", error);
      return [];
    }
  });
}
